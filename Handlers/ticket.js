import Discord, { Events, EmbedBuilder, ChannelType, ButtonBuilder, ActionRowBuilder, ComponentType, TextInputStyle, ButtonStyle, StringSelectMenuBuilder, ModalBuilder, TextInputBuilder } from "discord.js";
...
export const closeAllTickets = async (guild, author) => {
...
export const ticketModalSetup = async ({ channel, member }) => {
...
/**  */
export async function close({ channel }, author) {
...
/** */
export async function removeFromTicket({ channel }, inputId) {
    if (!isTicketChannel(channel)) return "Lệnh này chỉ có thể được sử dụng trong kênh ticket";
...
export default function ticketCreate(client) {

...
export default class components {
  ...
  /**
   * @info lấy đường dẫn từ file
   */
  globalFilePath(path) {
    return pathToFileURL(path)?.href || path;
  };
  /**
   * Hiển thị thông tin trạng thái của lệnh
   * @example className.logStatus(options)
   */
  logStatus(name, type, isLoaded) {
    const now = new Date();
    const date = now.toLocaleDateString("vi-VN");
    const time = now.toLocaleTimeString("vi-VN");
    const statusIcon = isLoaded ? "\x1b[32m✅\x1b[0m" : "\x1b[31m❌\x1b[0m";
    const statusText = isLoaded ? "Sẵn sàng." : "Sảy ra lỗi.";
    console.log(`[${date} ${time}] ${type}: ${name} | Trạng thái: ${statusIcon} ${statusText}`);
  };
};

export {
  components as classComponent
};